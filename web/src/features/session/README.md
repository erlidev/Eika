...
- `ContextMeter.tsx` is what the status bar states about cost. Every number in
  ...
  replay restores the meter. There is no decode rate: a Chat Completions
  endpoint reports its token counts when a response ends, never while one
  streams, so a tok/s shown during a turn could only be a guess. The tooltips
  state the numbers and leave the explaining to the run panel.
