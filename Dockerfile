...
COPY deploy/eika-entrypoint.sh /usr/local/bin/eika-entrypoint
EXPOSE 8080
# The entry point starts as root to join the Docker socket's group, whose id
# differs between hosts, then runs the harness as the non-root user eika.
# Membership in that group is root-equivalent on the host; the harness needs
# it to run sandboxes as sibling containers.
ENTRYPOINT ["/usr/local/bin/eika-entrypoint", "/usr/local/bin/eika"]
CMD ["-web", "/app/web"]
